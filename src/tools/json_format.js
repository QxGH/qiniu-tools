...
const transitionJsonToString = (jsonObj, callback) => {
  ...
  if (Object.prototype.toString.call(jsonObj) !== "[object String]") {
    try {
      ...
      // 转换失败错误信息
      console.log('您传递的json数据格式有误，请核对...');
      console.log(error);
      callback(error);
    }
  } else {
    try {
      ...
      let jsonP = JSON.parse(jsonObj)
      _jsonObj = JSON.stringify(jsonP);
    } catch (error) {
      // 转换失败错误信息
      console.log('您传递的json数据格式有误，请核对...');
      console.log(error);
...
const formatJson = (jsonObj, success, error) => {
  // 正则表达式匹配规则变量
  var reg = null;
  // 转换后的字符串变量
  var formatted = '';
  ...
  var PADDING = '   ';
  // json对象转换为字符串变量
  var jsonString = transitionJsonToString(jsonObj, error);
  if (!jsonString) {
    return jsonString;
  }
  // 存储需要特殊处理的字符串段
